$(document).ready( function () {
	...
	function make_alert() {
		...
		function load_cat() {
			...
			xhttp.open("GET", 'action.php?load_cat=1', true);
			xhttp.send();
			//xhttp.close();
		...
		function load_products() {
			...
			xhttp.open("GET","action.php?load_products=1", true);
			xhttp.send();
	...
	$('#cart_page_items_container').on('click', '.cart_page_product_quantity_update', function(event) {
		...
		$.ajax({
			...
			method: 'post',
			data: {
				update_cart_page_item_quantity: 1,
				id:id,
				quantity: quantity
			},
			success: function(res) {
